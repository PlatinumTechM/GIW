...
const FIELD_MAPPINGS = {
  ...
  jewelry_sub_category: ["category", "sub category", "sub-category", "item type", "jewellery sub category"],
  ...
  center_gem_type: ["center stone type", "center gem", "stone type", "diamond type", "center gemtype"],
  center_gem_color: ["center gem color", "stone color", "center gem type color"],
  center_shape: ["center shape", "stone shape", "diamond shape"],
  center_weight_cts: ["center weight", "stone weight", "diamond weight", "ct", "cts", "center weight (cts)"],
  center_color_grade: ["center color", "color grade", "color", "center color (d to z)"],
  ...
  center_fluorescence_intensity: ["center fluorescence", "fluorescence", "fluor", "center fluorescence intensity"],
  center_lab: ["center lab", "lab", "center lab"],
  center_enhancement: ["center enhancement", "treatment"],
  center_total_stones: ["center stones count", "center stones", "center total stones"],
  center_measurement_length: ["center length", "length", "center measurement length"],
  center_measurement_width: ["center width", "width", "center measurement width"],
  center_measurement_depth: ["center depth", "depth", "center measurement depth"],
  ...
  side_weight_cts: ["side weight", "side stone weight", "side diamond weight", "side weight (cts)"],
  ...
  setting_supported_carat_from: ["setting supported carat from", "supported carat from"],
  setting_supported_carat_to: ["setting supported carat to", "supported carat to"],
  supported_diamond_shapes: ["supported diamond shapes"],
  total_number_of_stones: ["total number of stones"],
  
  certificate_number: ["certificate number", "cert no", "report no", "certificate no"],
  ...
  const processFile = useCallback((uploadedFile) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const validData = mappedData.filter(row => row.stock_id && String(row.stock_id).trim() !== "");
        const skippedCount = mappedData.length - validData.length;

        if (validData.length === 0) {
          notify.error("No Valid Data", "No rows with a valid Stock ID were found.");
          return;
        }
        ...
        if (skippedCount > 0) {
          notify.warning("Data Filtered", `Loaded ${validData.length} items. Skipped ${skippedCount} rows missing Stock ID.`);
        } else {
          notify.success("File Parsed", `Loaded ${validData.length} items from ${uploadedFile.name}`);
        }
      } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const replaceMsg = result.updatedCount > 0 ? `, updated ${result.updatedCount} existing` : "";
        notify.success("Import Complete", `Saved ${result.insertedCount} rows${replaceMsg}, skipped ${result.skippedCount} rows`);
        if (onComplete) onComplete();
      }
    } catch (error) {
      console.error("Import error:", error);
      notify.error("Import Failed", error.response?.data?.message || "Failed to upload stock");
    } finally {
    ...
    <div className="bg-white rounded-3xl border border-[#E2E8F0] overflow-hidden shadow-sm">
      <div className="p-6 border-b border-[#E2E8F0] flex items-center justify-between bg-gradient-to-r from-slate-50 to-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-black text-[#0F172A] tracking-tight">Bulk Import Jewellery</h2>
            <p className="text-[10px] text-[#64748B] font-bold uppercase tracking-widest">Excel / CSV Data reconciliation</p>
          </div>
        </div>
        <button onClick={onCancel} className="p-2 text-[#94A3B8] hover:text-[#0F172A] transition-colors">
          <X className="w-5 h-5" />
        </button>
      </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              >
                <div className="flex flex-col items-center text-center">
                  ...
                  <p className="text-xs text-[#64748B]">Upload .xlsx, .xls, or .csv</p>
                </div>
              </div>
              ...
              >
                <div className="flex flex-col items-center text-center">
                  ...
                  <p className="text-xs text-[#64748B]">Download recommended format</p>
                </div>
              </div>

              <div
                className="bg-white rounded-2xl border-2 border-dashed border-slate-200 p-6 cursor-not-allowed opacity-60"
              >
                <div className="flex flex-col items-center text-center">
                  <div className="w-14 h-14 rounded-2xl bg-slate-50 flex items-center justify-center mb-3 text-slate-400">
                    <RotateCcw className="w-7 h-7" />
                  </div>
                  <h3 className="font-bold text-[#0F172A] mb-1">Coming Soon</h3>
                  <p className="text-xs text-[#64748B]">API Integration</p>
                </div>
              </div>
            </div>
            ...
            <div
              ...
              className={`relative h-48 rounded-2xl border-2 border-dashed transition-all flex flex-col items-center justify-center gap-3 ${
                isDragging ? "border-emerald-500 bg-emerald-50 scale-[0.99]" : "border-slate-200 bg-slate-50 hover:bg-slate-100/50 hover:border-emerald-300"
              }`}
            >
              ...
              <div className="text-center">
                <p className="text-sm font-bold text-[#0F172A]">Drop your file here, or <span className="text-emerald-600">browse</span></p>
                <p className="text-[10px] text-[#64748B] mt-1 uppercase font-black tracking-widest">CSV, XLSX, XLS up to 10MB</p>
              </div>
              <input type="file" ref={fileInputRef} onChange={handleFileChange} accept=".xlsx,.xls,.csv" className="absolute inset-0 opacity-0 cursor-pointer" />
            </div>
          </div>
        ) : (
          <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-5 bg-emerald-50 border border-emerald-100 rounded-2xl gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm font-black text-emerald-900 uppercase tracking-tight">{file.name}</p>
                  <p className="text-xs font-bold text-emerald-700">{data.length} items ready to import</p>
                </div>
              </div>

              <div className="flex flex-wrap items-center gap-3">
                <button onClick={() => { setFile(null); setData([]); }} className="px-4 py-2 text-rose-600 font-black text-[10px] uppercase tracking-widest hover:bg-rose-50 rounded-lg transition-all">
                  Remove
                </button>
              </div>
            </div>
                ...
                <thead className="sticky top-0 bg-slate-800 text-white z-10">
                  <tr>
                    {Object.keys(FIELD_MAPPINGS).map((header) => (
                      <th key={header} className="px-4 py-3 text-left font-black uppercase tracking-wider border-r border-white/10 whitespace-nowrap">
                        {header.replace(/_/g, ' ')}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {data.slice(0, 50).map((row, i) => (
                    <tr key={i} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                      {Object.keys(FIELD_MAPPINGS).map((header) => (
                        <td key={header} className="px-4 py-2.5 text-slate-700 truncate max-w-[200px] border-r border-slate-50">
                          {row[header] || "-"}
                        </td>
                      ))}
                    </tr>
