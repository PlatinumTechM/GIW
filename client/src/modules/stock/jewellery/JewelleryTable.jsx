  ...
  const SimpleTableRow = ({ item, index, page }) => {
    ...
    return (
      ...
      >
        ...
        <td
          className={`px-2 py-3 text-center text-xs text-[#64748B] border-r border-[#E2E8F0] sticky left-0 z-20 w-16 font-medium ${rowBgClass} ${isSelected ? "bg-blue-50" : ""}`}
        >
          <div
        ...
        {/* Fields */}
        {fields.map((field) => {
          // Handle boolean values specially - don't treat false as empty
          const isBoolean = typeof item[field] === "boolean";
          const value = isBoolean ? item[field] : item[field] || "-";
          const isEmpty = value === "-";
          ...
          return (
            ...
            >
              {field === "stock_id" && !isEmpty ? (
                ...
                <span
                  className={`px-2 py-0.5 text-[10px] font-black rounded-full border shadow-sm uppercase tracking-wider ${
              ...
              ) : (field.includes("weight") || field.includes("carat")) &&
                !isEmpty ? (
        ...
        {/* Actions */}
        <td
          className="px-2 py-2 border-r border-[#E2E8F0] text-center sticky right-0 bg-white shadow-[-2px_0_4px_rgba(0,0,0,0.05)] z-20"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="flex items-center justify-center gap-1">
            ...
            <button
              ...
              className="p-1.5 bg-rose-100 text-rose-600 rounded-full hover:bg-rose-200 transition-all"
              title="Delete"
            >
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-[#E2E8F0] p-4 flex items-center justify-between shadow-sm">
        <span className="text-sm text-[#64748B] font-medium">
          Showing{" "}
          <span className="font-bold text-[#0F172A]">{data.length}</span> items
          {pagination && (
            <>
              {" "}
              of{" "}
          ...
          <thead className="sticky top-0 z-40 bg-gray-200">
            <tr className="bg-gray-200 text-gray-900">
              ...
              {fields.map((field) => (
                <th
                  key={field}
                  className="px-3 py-3 text-left border-r border-gray-300 whitespace-nowrap text-xs font-black uppercase bg-gray-200 text-slate-700 tracking-wider"
                >
                  {field.replace(/_/g, " ")}
                </th>
              ))}
              <th className="px-3 py-3 text-center border-r border-gray-300 bg-gray-200 text-xs font-black uppercase text-slate-700 tracking-wider">
                Media
              </th>
              <th className="px-3 py-3 text-center sticky right-0 z-50 bg-gray-300 border-l border-gray-300 text-xs font-black uppercase text-slate-700 tracking-wider w-24">
                Actions
              </th>
            </tr>
          </thead>
            ...
            {data.length === 0 ? (
              <tr>
                <td
                  ...
                  className="px-6 py-20 text-center text-[#64748B] font-bold uppercase tracking-[0.2em] text-xs"
                >
