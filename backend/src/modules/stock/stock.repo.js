...
const ALL_COLUMNS = [
  ...
  "type", "user_id", "stock_id", "certificate_number", "weight",

  "shape", "color", "fancy_color", "fancy_color_intensity", "fancy_color_overtone",

  "clarity", "cut", "polish", "symmetry", "fluorescence", "fluorescence_color", "fluorescence_intensity",
  ...
  "rap_per_carat", "price_per_carat", "final_price", "dollar_rate", "rs_amount", "discount",

  "heart_arrow", "star_length", "laser_description", "growth_type", "key_to_symbol", "lw_ratio",
...
const bulkInsert = async (stockDataArray) => {
  ...
  const placeholders = stockDataArray
    ...
    .map((_, rowIndex) => {
      ...
      const rowPlaceholders = columnsWithData
        ...
        .map((_, colIndex) => `$${rowIndex * columnsWithData.length + colIndex + 1}`)
...
const deleteByStockIds = async (stockIds) => {
  ...
  const result = await pool.query(query, stockIds);
...
// Check which stock_ids already exist (for warning before replace)

const checkExistingStockIds = async (stockIds) => {
  ...
  const query = `SELECT stock_id FROM diamond_stock WHERE stock_id IN (${placeholders})`;
...
const getByUserId = async (userId, page = 1, limit = 50) => {
